 ...
 * 
 * DESCRIPTION:
 * Create a new product with bilingual data, images, variants, and categories.
 * Inline brand/category creation modals.
 * 
 * FUNCTIONALITY:
 * - Bilingual name + description (FR/AR)
 ...
 * 
 * BACKEND INTEGRATION:
 * - GET /api/admin/brands — for brand dropdown
 * - GET /api/admin/categories — for category checkboxes
 * - POST /api/upload — Cloudinary image upload
 * - POST /api/admin/products — create product with all relations
 * - POST /api/admin/brands — inline brand creation
 * - POST /api/admin/categories — inline category creation
 */
...
import { ArrowLeft, Save, Plus, Trash2, Upload, X, Star, ChevronUp, ChevronDown, Image as ImageIcon, Package } from 'lucide-react'
...
export default function NewProductPage() {
  ...
  const [formData, setFormData] = useState<FormData>({
    ...
    variants: [{ size: '', price: '', stock: '' }], images: [],
  })
  ...
  const addVariant = () => setFormData(p => ({ ...p, variants: [...p.variants, { size: '', price: '', stock: '' }] }))
  const removeVariant = (i: number) => { if (formData.variants.length > 1) setFormData(p => ({ ...p, variants: p.variants.filter((_, idx) => idx !== i) })) }
  const updateVariant = (i: number, field: keyof Variant, value: string) => { const v = [...formData.variants]; v[i][field] = value; setFormData(p => ({ ...p, variants: v })) }
  ...
  const handleFileSelect = (e: React.ChangeEvent<HTMLInputElement>) => { const files = e.target.files; if (files) Array.from(files).forEach(f => uploadImage(f)); e.target.value = '' }
  ...
  const validateForm = (): boolean => {
    ...
    formData.variants.forEach((v, i) => {
      ...
      if (!v.price || parseFloat(v.price) <= 0) e[`variant_${i}_price`] = isRtl ? 'سعر صحيح مطلوب' : 'Valid price required'
      if (!v.stock || parseInt(v.stock) < 0) e[`variant_${i}_stock`] = isRtl ? 'مخزون صحيح مطلوب' : 'Valid stock required'
    })
    setErrors(e)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/admin/products', {
        ...
        body: JSON.stringify({ ...formData, variants: formData.variants.map(v => ({ size: v.size, price: parseFloat(v.price), stock: parseInt(v.stock) })) }),
      })
      if (res.ok) { toast.success(isRtl ? 'تم إنشاء المنتج' : 'Produit créé'); router.push('/admin/products') }
      else { const d = await res.json(); toast.error(d.error || tc('error')) }
    } catch { toast.error(tc('error')) }
    setLoading(false)
  }
      ...
      <form onSubmit={handleSubmit} style={{ maxWidth: 800 }}>
        <div style={{ backgroundColor: T.cream, border: `1px solid ${T.gold}12`, padding: 28, display: 'flex', flexDirection: 'column', gap: 28 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
                ...
                {[
                  ...
                  <div key={f.key}>
                    ...
                    <input type="text" value={(formData as any)[f.key]} onChange={e => setFormData(p => ({ ...p, [f.key]: e.target.value }))}
                      ...
                      onBlur={e => { if (!errors[f.key]) e.currentTarget.style.borderColor = `${T.espresso}12` }} />
              ...
              <div className="grid grid-cols-1 md:grid-cols-2" style={{ gap: 16 }}>
                <div>
                  <div className="flex items-center justify-between" style={{ marginBottom: 6 }}>
                    <span style={{ fontFamily: 'Inter, sans-serif', fontSize: 10, letterSpacing: '0.2em', textTransform: 'uppercase', color: T.muted, fontWeight: 500 }}>
                      {tp('name') === 'Nom' ? 'Marque' : 'العلامة التجارية'} *
                    </span>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                <label className="flex items-center" style={{ gap: 8, cursor: 'pointer' }}>
                  <input type="checkbox" checked={formData.isFeatured} onChange={e => setFormData(p => ({ ...p, isFeatured: e.target.checked }))} style={{ width: 16, height: 16, accentColor: T.gold }} />
                  <span style={{ fontFamily: 'Inter, sans-serif', fontSize: 12, color: T.espresso, fontWeight: 300 }}>{tp('isFeatured')}</span>
                </label>
                <label className="flex items-center" style={{ gap: 8, cursor: 'pointer' }}>
                  <input type="checkbox" checked={formData.isNewArrival} onChange={e => setFormData(p => ({ ...p, isNewArrival: e.target.checked }))} style={{ width: 16, height: 16, accentColor: T.gold }} />
                  <span style={{ fontFamily: 'Inter, sans-serif', fontSize: 12, color: T.espresso, fontWeight: 300 }}>{tp('isNewArrival')}</span>
                </label>
                <label className="flex items-center" style={{ gap: 8, cursor: 'pointer' }}>
                  <input type="checkbox" checked={formData.isActive} onChange={e => setFormData(p => ({ ...p, isActive: e.target.checked }))} style={{ width: 16, height: 16, accentColor: T.gold }} />
                  <span style={{ fontFamily: 'Inter, sans-serif', fontSize: 12, color: T.espresso, fontWeight: 300 }}>{tp('active')}</span>
                </label>
              </div>
            </div>
          </div>
          ...
          <div>
            ...
            <div onDragEnter={e => { e.preventDefault(); setDragActive(true) }} onDragLeave={e => { e.preventDefault(); setDragActive(false) }} onDragOver={e => e.preventDefault()} onDrop={e => { e.preventDefault(); setDragActive(false); if (e.dataTransfer.files) Array.from(e.dataTransfer.files).forEach(f => uploadImage(f)) }}
              ...
              <input type="file" id="imgUpload" multiple accept="image/*" onChange={handleFileSelect} disabled={uploading || formData.images.length >= 10} className="sr-only" />
              <label htmlFor="imgUpload" style={{ cursor: 'pointer', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8 }}>
                {uploading ? (
                  <div style={{ width: 36, height: 36, border: `3px solid ${T.gold}`, borderTopColor: 'transparent', borderRadius: '50%', animation: 'spin 0.8s linear infinite' }} />
                ) : (
                  <>
                    <Upload style={{ width: 36, height: 36, color: `${T.muted}40` }} />
                    <p style={{ fontFamily: 'Inter, sans-serif', fontSize: 12, color: T.muted, fontWeight: 300 }}>{tp('dragDrop')}</p>
                    <p style={{ fontFamily: 'Inter, sans-serif', fontSize: 9, color: `${T.muted}60`, fontWeight: 300 }}>PNG, JPG, WebP · 5MB max</p>
                  </>
                )}
              </label>
            </div>
                ...
                {formData.images.map((img, i) => (
                  <div key={i} className="relative group" style={{ aspectRatio: '3/4', backgroundColor: T.dust, border: `1px solid ${T.gold}10` }}>
                    {i === 0 && <div style={{ position: 'absolute', top: 6, [isRtl ? 'right' : 'left']: 6, zIndex: 10, backgroundColor: T.gold, color: T.espresso, padding: '1px 8px', fontFamily: 'Inter, sans-serif', fontSize: 8, fontWeight: 500, display: 'flex', alignItems: 'center', gap: 4 }}>
                      <Star style={{ width: 10, height: 10 }} /> {tp('name') === 'Nom' ? 'Principale' : 'رئيسية'}
                    </div>}
                    ...
                    <div className="absolute inset-0 flex items-center justify-center" style={{ gap: 4, opacity: 0, transition: 'opacity 0.2s', backgroundColor: `${T.espresso}60` }}
                      ...
                      <button type="button" onClick={() => moveImageUp(i)} disabled={i === 0}
                        style={{ padding: 6, backgroundColor: T.cream, border: 'none', borderRadius: '50%', cursor: 'pointer', opacity: i === 0 ? 0.3 : 1, color: T.espresso }}>
                        <ChevronUp style={{ width: 14, height: 14 }} />
                      </button>
                      {i !== 0 && <button type="button" onClick={() => setPrimaryImage(i)} style={{ padding: 6, backgroundColor: T.cream, border: 'none', borderRadius: '50%', cursor: 'pointer', color: T.gold }}>
                        <Star style={{ width: 14, height: 14 }} />
                      </button>}
                      <button type="button" onClick={() => moveImageDown(i)} disabled={i === formData.images.length - 1}
                        style={{ padding: 6, backgroundColor: T.cream, border: 'none', borderRadius: '50%', cursor: 'pointer', opacity: i === formData.images.length - 1 ? 0.3 : 1, color: T.espresso }}>
                        <ChevronDown style={{ width: 14, height: 14 }} />
                      </button>
                      <button type="button" onClick={() => removeImage(i)} style={{ padding: 6, backgroundColor: T.cream, border: 'none', borderRadius: '50%', cursor: 'pointer', color: '#C0392B' }}>
              ...
              {formData.variants.map((v, i) => (
                <div key={i} className="grid grid-cols-1 md:grid-cols-12" style={{ gap: 10, padding: 14, border: `1.5px solid ${T.espresso}10` }}>
                  <div style={{ gridColumn: 'span 4' }}>
                    ...
                    <input type="text" value={v.size} onChange={e => updateVariant(i, 'size', e.target.value)} placeholder={isRtl ? 'مثال: 50مل' : 'e.g., 50ml'}
                      ...
                      onFocus={e => { e.currentTarget.style.borderColor = T.gold }}
                      onBlur={e => { if (!errors[`variant_${i}_size`]) e.currentTarget.style.borderColor = `${T.espresso}12` }} />
                  ...
                  <div style={{ gridColumn: 'span 3' }}>
                    ...
                    <input type="number" step="0.01" value={v.price} onChange={e => updateVariant(i, 'price', e.target.value)}
                      ...
                      onFocus={e => { e.currentTarget.style.borderColor = T.gold }}
                      onBlur={e => { if (!errors[`variant_${i}_price`]) e.currentTarget.style.borderColor = `${T.espresso}12` }} />
                  ...
                  <div style={{ gridColumn: 'span 3' }}>
                    ...
                    <input type="number" value={v.stock} onChange={e => updateVariant(i, 'stock', e.target.value)}
                      ...
                      onFocus={e => { e.currentTarget.style.borderColor = T.gold }}
                      onBlur={e => { if (!errors[`variant_${i}_stock`]) e.currentTarget.style.borderColor = `${T.espresso}12` }} />
          ...
          <div className="flex" style={{ gap: 12, paddingTop: 8, borderTop: `1px solid ${T.gold}10` }}>
            <button type="submit" disabled={loading}
              style={{ padding: '12px 28px', fontFamily: 'Inter, sans-serif', fontSize: 10, letterSpacing: '0.2em', textTransform: 'uppercase', border: 'none', cursor: loading ? 'not-allowed' : 'pointer', backgroundColor: T.espresso, color: T.ivory, fontWeight: 500, transition: 'background-color 0.3s', opacity: loading ? 0.6 : 1, display: 'flex', alignItems: 'center', gap: 8 }}
              onMouseEnter={e => { if (!loading) e.currentTarget.style.backgroundColor = T.gold }}
              onMouseLeave={e => { if (!loading) e.currentTarget.style.backgroundColor = T.espresso }}>
              <Save style={{ width: 16, height: 16 }} />
              {loading ? tp('creating') : tp('save')}
            </button>
        ...
        {showBrandModal && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }}
              ...
              <button onClick={() => setShowBrandModal(false)} style={{ position: 'absolute', top: 16, [isRtl ? 'left' : 'right']: 16, background: 'none', border: 'none', cursor: 'pointer', color: T.muted }}>
                <X style={{ width: 18, height: 18 }} />
              </button>
              <h2 style={{ fontFamily: 'Cormorant Garamond, Georgia, serif', fontSize: '1.2rem', fontWeight: 300, color: T.espresso, marginBottom: 20 }}>
                {tc('add')} — {isRtl ? 'علامة تجارية' : 'Marque'}
              </h2>
              <form onSubmit={handleCreateBrand}>
                <div style={{ marginBottom: 20 }}>
                  <label style={{ display: 'block', fontFamily: 'Inter, sans-serif', fontSize: 10, letterSpacing: '0.2em', textTransform: 'uppercase', color: T.muted, marginBottom: 6, fontWeight: 500 }}>
                    {isRtl ? 'اسم العلامة التجارية' : 'Nom de la marque'} *
                  </label>
                ...
                <div className="flex" style={{ gap: 10 }}>
                  <button type="submit" disabled={savingBrand} style={{ flex: 1, padding: '11px 20px', fontFamily: 'Inter, sans-serif', fontSize: 10, letterSpacing: '0.2em', textTransform: 'uppercase', border: 'none', cursor: savingBrand ? 'not-allowed' : 'pointer', backgroundColor: T.espresso, color: T.ivory, fontWeight: 500, opacity: savingBrand ? 0.6 : 1 }}
                    onMouseEnter={e => { if (!savingBrand) e.currentTarget.style.backgroundColor = T.gold }}
                    onMouseLeave={e => { if (!savingBrand) e.currentTarget.style.backgroundColor = T.espresso }}>
                    {savingBrand ? tc('saving') + '...' : tc('save')}
                  </button>
                  <button type="button" onClick={() => setShowBrandModal(false)} style={{ padding: '11px 20px', fontFamily: 'Inter, sans-serif', fontSize: 10, letterSpacing: '0.2em', border: `1.5px solid ${T.espresso}15`, backgroundColor: 'transparent', color: T.espresso, cursor: 'pointer' }}>
                    {tc('cancel')}
                  </button>
                </div>
              </form>
      ...
      <AnimatePresence>
        {showCategoryModal && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }}
              style={{ backgroundColor: T.cream, maxWidth: 420, width: '100%', padding: 28, position: 'relative' }}>
              <button onClick={() => setShowCategoryModal(false)} style={{ position: 'absolute', top: 16, [isRtl ? 'left' : 'right']: 16, background: 'none', border: 'none', cursor: 'pointer', color: T.muted }}>
                <X style={{ width: 18, height: 18 }} />
              </button>
              <h2 style={{ fontFamily: 'Cormorant Garamond, Georgia, serif', fontSize: '1.2rem', fontWeight: 300, color: T.espresso, marginBottom: 20 }}>
                {tc('add')} — {isRtl ? 'فئة' : 'Catégorie'}
              </h2>
              <form onSubmit={handleCreateCategory}>
                <div style={{ marginBottom: 20 }}>
                  <label style={{ display: 'block', fontFamily: 'Inter, sans-serif', fontSize: 10, letterSpacing: '0.2em', textTransform: 'uppercase', color: T.muted, marginBottom: 6, fontWeight: 500 }}>
                    {isRtl ? 'اسم الفئة' : 'Nom de la catégorie'} *
                  </label>
                ...
                <div className="flex" style={{ gap: 10 }}>
                  <button type="submit" disabled={savingCategory} style={{ flex: 1, padding: '11px 20px', fontFamily: 'Inter, sans-serif', fontSize: 10, letterSpacing: '0.2em', textTransform: 'uppercase', border: 'none', cursor: savingCategory ? 'not-allowed' : 'pointer', backgroundColor: T.espresso, color: T.ivory, fontWeight: 500, opacity: savingCategory ? 0.6 : 1 }}
                    onMouseEnter={e => { if (!savingCategory) e.currentTarget.style.backgroundColor = T.gold }}
                    onMouseLeave={e => { if (!savingCategory) e.currentTarget.style.backgroundColor = T.espresso }}>
                    {savingCategory ? tc('saving') + '...' : tc('save')}
                  </button>
                  <button type="button" onClick={() => setShowCategoryModal(false)} style={{ padding: '11px 20px', fontFamily: 'Inter, sans-serif', fontSize: 10, letterSpacing: '0.2em', border: `1.5px solid ${T.espresso}15`, backgroundColor: 'transparent', color: T.espresso, cursor: 'pointer' }}>
                    {tc('cancel')}
                  </button>
                </div>
              </form>
