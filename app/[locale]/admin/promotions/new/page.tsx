/**
 * Admin New Promotion Page
 * 
 * DESCRIPTION:
 * Create a new promotional pricing — set name, discount, dates, and select variants.
 * 
 * FUNCTIONALITY:
 * - Name, discount type (% or fixed), discount value
 * - Start/end date pickers
 * - Active toggle
 * - Searchable variant selector with product details
 * - Form validation before submit
 * 
 * BACKEND INTEGRATION:
 * - GET /api/admin/variants — load all variants for selection
          ...
          <div>
            ...
            <div style={{ border: `1.5px solid ${errors.variantIds ? '#C0392B' : `${T.espresso}12`}`, maxHeight: 460, overflowY: 'auto' }}>
              {filtered.length === 0 ? (
                ...
                filtered.map(v => (
                  <label key={v.id} className="flex items-center" style={{ gap: 12, padding: 12, borderBottom: `1px solid ${T.gold}08`, cursor: 'pointer', transition: 'background-color 0.15s' }}
