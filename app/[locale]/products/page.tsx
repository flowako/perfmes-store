...
function FilterPanel({ filters, onChange, onReset, count, availableBrands, availableCategories, availableSizes, t }:{
  ...
  const toggleGender = (gender: Gender) => {
    const newGenders = new Set(filters.genders);
    if (newGenders.has(gender)) newGenders.delete(gender);
    else newGenders.add(gender);
  ...
  return (
    <div>
      ...
      <FilterSection title={t('for')}>
        {GENDERS.map(g => {
          const genderLabel = g === "MEN" ? t('filters.gender.men') : g === "WOMEN" ? t('filters.gender.women') : t('filters.gender.unisex');
          return (
            <Checkbox key={g} label={genderLabel} checked={filters.genders.has(g)}
...
function ActiveChips({ filters, onChange, t }:{ filters:Filters; onChange:(f:Filters)=>void; t: (key:string) => string }) {
  ...
  filters.genders.forEach(g => {
    const label = g === "MEN" ? t('filters.gender.men') : g === "WOMEN" ? t('filters.gender.women') : t('filters.gender.unisex');
    chips.push({ label, remove:()=>onChange({...filters, genders:new Set([...filters.genders].filter(x=>x!==g))}) });
  });
...
function PageBanner({ t }: { t: (key:string) => string }) {
  return (
...
function paramsToFilters(searchParams: URLSearchParams): { filters: Filters; sort: SortKey; search: string } {
  const filters: Filters = {
    ...
    genders: new Set(searchParams.getAll('gender').filter(g => ['MEN','WOMEN','UNISEX'].includes(g)) as Gender[]),
...
const DEFAULT_FILTERS: Filters = {
  brands: new Set(), categories: new Set(), genders: new Set(),
...
function ProductsPage() {
  ...
  const activeFilterCount =
    filters.brands.size + filters.categories.size + filters.genders.size + filters.sizes.size +
